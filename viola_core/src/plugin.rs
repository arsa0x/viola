...
impl Host for PluginHost {
    async fn send_text(&self, text: &str) -> Result<(), viola_script::error::NativeError> {
        self.ctx.send().text(text).await.map_err(|e| NativeError {
            kind: viola_script::error::NativeErrorKind::HostRejected,
            detail: e.to_string(),
        })
    }
}
